...
# TODO http://dweb.archive.org/metadata/xxx -> gateway /arc/archive.org/metadata  etc
...
server {
    # dweb.archive.org answers on 80, dweb.me and gateway.dweb.me on 443
	listen 80;
...
# On dweb.me or gateway.dweb.me redirect to https://dweb.archive.org so client uses https, most of these are going to be crawlers/hackers
...
#    }
	...
	location / {
		rewrite (.*)  /arc/archive.org/$1 break;
        proxy_set_header        Host $host;
        proxy_set_header        X-Real-IP $remote_addr;
        proxy_set_header        X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header        X-Forwarded-Proto $scheme;
